<template>
	<div>
		<el-row v-show="showSearch">
			<el-form :model="state.queryForm" ref="queryRef" :inline="true" @keyup.enter="getDataList">
				<el-form-item :label="t('sms.configName')" prop="configName">
					<el-input :placeholder="t('sms.inputConfigNameTip')" v-model="state.queryForm.configName" />
				</el-form-item>
				<el-form-item>
		...
		>
			...
			<el-table-column prop="configName" :label="t('sms.configName')" show-overflow-tooltip />
			<el-table-column prop="configKey" :label="t('sms.configKey')" show-overflow-tooltip />
			<el-table-column prop="configValue.supplier" :label="t('sms.supplier')" show-overflow-tooltip>
				<template #default="scope">
					{{ formDialogRef?.supplierList.find((item: any) => item.value === JSON.parse(scope.row.configValue).supplier)?.label }}
				</template>
			</el-table-column>
			<el-table-column prop="configStatus" :label="t('sms.configStatus')" show-overflow-tooltip width="100">
				<template #default="scope">
...
const handleDelete = async (ids: string[]): Promise<void> => {
	...
	try {
		await delObjs(ids);
		getDataList();
		useMessage().success(t('sms.deleteSuccess'));
	} catch (err: any) {
