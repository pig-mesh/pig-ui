<template>
	<div class="min-h-full p-6 bg-slate-100 dark:bg-slate-900">
		<!-- 骨架屏 -->
		<div v-if="pageLoading" class="animate-pulse">
			...
			<div class="grid gap-4 charts-grid">
				<div class="h-[420px] rounded-2xl bg-white dark:bg-slate-800" />
				<div class="h-[420px] rounded-2xl bg-white dark:bg-slate-800" />
			</div>
		</div>
		...
		<template v-else>
			...
			<!-- 统计卡片 -->
			<div class="mb-5 grid grid-cols-2 gap-3.5 md:grid-cols-3 xl:grid-cols-6">
				<div v-for="(card, i) in statisticsCards" :key="i"
					...
					<div class="flex items-center gap-2 mb-2">
						<div
							class="flex items-center justify-center rounded-lg h-7 w-7 shrink-0 bg-indigo-50 dark:bg-indigo-950">
							<svg class="h-3.5 w-3.5 text-indigo-600 dark:text-indigo-400" viewBox="0 0 24 24"
								fill="currentColor">
			...
			<div class="grid items-stretch gap-4 charts-grid">
				...
				<!-- 命令统计 -->
				<div
					class="overflow-hidden bg-white border shadow-sm rounded-2xl border-slate-100 dark:border-slate-700 dark:bg-slate-800">
					<div
						class="flex items-center justify-between border-b border-slate-100 px-5 py-[18px] dark:border-slate-700">
						<span
							class="border-l-[3px] border-indigo-700 pl-2.5 text-[15px] font-semibold text-slate-900 dark:text-slate-100">
							命令统计
						</span>
						<span class="text-[11px] text-slate-400 dark:text-slate-500">
							实时 {{ baseInfo.instantaneous_ops_per_sec || 0 }} ops/s
						</span>
					</div>
					<div class="px-5 pt-2">
						<v-chart class="h-[200px] w-full" :option="commandChartOption" autoresize />
					</div>
					<!-- 图例：仅展示 Top 8 -->
					<div class="flex flex-col gap-2 px-5 pt-1 pb-4">
						<div v-for="(item, i) in topCommands" :key="i" class="flex items-center gap-2.5 text-[13px]">
							<span class="w-2 h-2 rounded-full shrink-0"
								:style="{ background: commandColors[i % commandColors.length] }" />
							<span class="flex-1 text-slate-500 dark:text-slate-400">{{ item.name }}</span>
							<span class="text-xs font-semibold tabular-nums text-slate-900 dark:text-slate-100">
								{{ item.value.toLocaleString() }}
							</span>
							<span class="w-10 text-xs text-right text-slate-400 dark:text-slate-500">
								{{ getCommandPct(item.value) }}%
							</span>
						</div>
					</div>
					<div class="border-t border-slate-100 px-5 py-2.5 dark:border-slate-700">
						<span class="text-[11px] text-slate-400 dark:text-slate-500">
							共 {{ commandList.length }} 种命令 · 累计处理 {{ Number(baseInfo.total_commands_processed ||
								0).toLocaleString() }} 次
						</span>
					</div>
				</div>
				...
				<div
					...
					<!-- 半圆仪表盘 -->
					<div class="px-5 pt-2">
						<v-chart class="h-[200px] w-full" :option="memoryChartOption" autoresize />
					</div>
					<!-- 峰值 & Lua -->
					<div class="grid grid-cols-2 gap-3 px-5 pb-3">
						<div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
							<div
								class="mb-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
								历史峰值</div>
							<div class="text-sm font-bold text-slate-900 dark:text-slate-100">{{
								baseInfo.used_memory_peak_human || '-' }}</div>
							<div class="mt-0.5 text-[10px] text-slate-400">占峰值 {{ baseInfo.used_memory_peak_perc || '-'
							}}</div>
						</div>
						<div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
							<div
								...
								Lua 内存</div>
							<div class="text-sm font-bold text-slate-900 dark:text-slate-100">{{
								baseInfo.used_memory_lua_human || '-' }}</div>
							<div class="mt-0.5 text-[10px] text-slate-400">已缓存脚本 {{ baseInfo.number_of_cached_scripts ||
								'0' }} 个</div>
						</div>
					</div>
					<!-- 进度条：当前 vs 峰值 -->
					<div class="px-5 pb-3">
						<div
							class="mb-1 flex items-center justify-between text-[11px] text-slate-400 dark:text-slate-500">
							<span>当前占用 / 历史峰值</span>
					...
					<div class="grid grid-cols-2 gap-3 px-5 pb-3">
						<div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
							...
							<div class="mt-0.5 text-[10px] text-slate-400">命中 {{ Number(baseInfo.keyspace_hits ||
								0).toLocaleString() }} / 未中 {{ Number(baseInfo.keyspace_misses || 0).toLocaleString() }}
							</div>
						</div>
						<div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50">
							<div
								...
								已驱逐键</div>
							<div class="text-sm font-bold text-slate-900 dark:text-slate-100">{{
								Number(baseInfo.evicted_keys || 0).toLocaleString() }}</div>
							<div class="mt-0.5 text-[10px] text-slate-400">策略: {{ baseInfo.maxmemory_policy ||
								'noeviction' }}</div>
						</div>
					</div>
					<!-- 碎片率 & 订阅 -->
					<div
						class="flex items-center justify-between px-5 py-3 border-t border-slate-100 dark:border-slate-700">
						<div class="text-[11px] text-slate-500 dark:text-slate-400">
							...
							<span v-if="fragRatio >= 2"
								class="ml-1.5 rounded-full bg-rose-50 px-1.5 py-0.5 text-[10px] font-medium text-rose-500 dark:bg-rose-950 dark:text-rose-400">偏高</span>
						</div>
						<div class="text-[11px] text-slate-500 dark:text-slate-400">
...
const commandColors = [
	...
	'#c7d2fe', '#0284c7', '#0ea5e9', '#38bdf8', '#7dd3fc',
...
const usedMemoryMB = computed(() =>
	baseInfo.value.used_memory ? (baseInfo.value.used_memory / 1024 / 1024).toFixed(2) : '0'
);
const peakMemoryMB = computed(() =>
	baseInfo.value.used_memory_peak
		? (baseInfo.value.used_memory_peak / 1024 / 1024).toFixed(2)
		: usedMemoryMB.value
);
...
const statisticsCards = computed(() => {
	...
	return [
		...
		{
			iconPath: 'M12 3C7.58 3 4 4.79 4 7s3.58 4 8 4 8-1.79 8-4-3.58-4-8-4zM4 9v3c0 2.21 3.58 4 8 4s8-1.79 8-4V9c0 2.21-3.58 4-8 4s-8-1.79-8-4zm0 5v3c0 2.21 3.58 4 8 4s8-1.79 8-4v-3c0 2.21-3.58 4-8 4s-8-1.79-8-4z',
			label: '键空间',
			value: info.dbSize || '0',
