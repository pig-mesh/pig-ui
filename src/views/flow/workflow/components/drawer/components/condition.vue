...
const formList = computed(() => {
	...
	let $deepCopy = other.deepClone(
		value.filter(
			(res) => res.type != 'MultiSelect' && res.type != 'Description' && res.type != 'Layout' && res.type != 'UploadImage' && res.type != 'UploadFile'
		)
	);
...
<template>
	<div>
		...
		<div style="margin-top: 20px">
			<select-show v-if="formIdObj[condition.key]?.type === 'SelectUser'" v-model:orgList="condition.value" type="user" :multiple="true"></select-show>
		</div>
		<div style="margin-top: 20px">
			<select-show
