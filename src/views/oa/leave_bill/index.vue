			...
			<el-row v-show="showSearch" class="mb8">
				<el-form :model="state.queryForm" ref="queryRef" :inline="true" @keyup.enter="getDataList">
					<el-form-item :label="$t('leave.username')" prop="username">
						<el-input :placeholder="t('leave.inputUsernameTip')" v-model="state.queryForm.username" style="max-width: 180px" />
					</el-form-item>
					<el-form-item :label="t('leave.state')" prop="state" class="ml2">
						<el-select v-model="state.queryForm.state" :placeholder="t('leave.inputStateTip')">
							<el-option :label="item.label" :value="item.value" v-for="(item, index) in leave_status" :key="index"></el-option>
						</el-select>
					</el-form-item>
			...
			>
				...
				<el-table-column :label="$t('common.action')" width="150">
					<template #default="scope">
						...
						<el-button text type="primary" @click="formDialogRef.openDialog(scope.row.leaveId)">{{ $t('common.editBtn') }}</el-button>
						<el-button text type="primary" @click="handleDelete([scope.row.leaveId])">{{ $t('common.delBtn') }}</el-button>
					</template>
				</el-table-column>
			</el-table>
