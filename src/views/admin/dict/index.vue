    ...
    <div class="layout-padding">
        <div class="layout-padding-auto layout-padding-view">
            <el-row class="mb8" v-show="showSearch">
                <el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
                    <el-form-item :label="$t('sysdict.dictType')" prop="dictType">
                        <el-input :placeholder="$t('sysdict.inputDictTypeTip')" style="max-width: 180px"
            ...
            <el-table :data="state.dataList" @selection-change="handleSelectionChange" style="width: 100%"
                ...
                <el-table-column :selectable="handleSelectable" align="center" type="selection"
                ...
                <el-table-column :label="$t('sysdict.dictType')" show-overflow-tooltip>
                    <template #default="scope">
                        <el-button @click="showDictITem(scope.row)" text type="primary">{{ scope.row.dictType }}
                        </el-button>
                    </template>
                </el-table-column>
                <el-table-column :label="$t('sysdict.description')" prop="description" show-overflow-tooltip
                ...
                <el-table-column :label="$t('sysdict.createTime')" prop="createTime" show-overflow-tooltip
                                 sortable></el-table-column>
                <el-table-column :label="$t('common.action')" width="200">
                    <template #default="scope">
                        <el-button @click="showDictITem(scope.row)" text type="primary">{{ $t('sysdict.dictItem') }}
                        </el-button>
                        <el-button @click="onOpenEditDic('edit', scope.row)" text type="primary">{{ $t('common.editBtn')
                            }}
                        </el-button>
                        <el-tooltip :content="$t('sysdict.deleteDisabledTip')" :disabled="scope.row.systemFlag === 1"
                                    placement="top">
							<span style="margin-left: 12px">
								<el-button :disabled="scope.row.systemFlag !== 1" @click="handleDelete([scope.row.id])" text
                                           type="primary">
									{{ $t('common.delBtn') }}
								</el-button>
							</span>
