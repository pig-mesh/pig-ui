<template>
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			...
			<el-row>
				<div class="mb8" style="width: 100%">
					...
					<right-toolbar v-model:showSearch="showSearch" :export="'sys_role_export'"
						@exportExcel="exportExcel" class="ml10" style="float: right; margin-right: 20px"
						@queryTable="getDataList"></right-toolbar>
				</div>
			</el-row>
			<el-table :data="state.dataList" v-loading="state.loading" style="width: 100%" row-key="roleId"
				...
				<el-table-column :label="$t('common.action')" width="320">
					<template #default="scope">
						...
						<el-tooltip :content="$t('sysrole.deleteDisabledTip')" :disabled="scope.row.roleId !== '1'"
							placement="top">
							<span style="margin-left: 12px">
								<el-button text type="primary" icon="delete" :disabled="scope.row.roleId === '1'"
									v-auth="'sys_role_del'" @click="handleDelete([scope.row.roleId])">{{
										$t('common.delBtn') }}
								</el-button>
							</span>
						</el-tooltip>
		...
		<!-- 导入角色 -->
		<upload-excel ref="excelUploadRef" :title="$t('sysrole.importRoleTip')" url="/admin/role/import"
			temp-url="/admin/sys-file/local/file/role.xlsx" @refreshDataList="getDataList" />
		<!-- 授权 -->
		<permession ref="permessionRef" />
		<!-- 首页配置 -->
		<RoleWidgetConfig ref="roleWidgetRef" />
	</div>
</template>
...
import RoleWidgetConfig from './widget.vue';
const { t } = useI18n();
