<template>
	<div style="font-family: 'IBM Plex Sans', system-ui, -apple-system, sans-serif;">
		<!-- 优雅的分割线 -->
		<div class="flex items-center my-8">
			<div class="flex-1 h-px bg-gray-200 dark:bg-slate-600"></div>
			<span class="px-4 text-xs text-gray-400 dark:text-slate-500 font-medium tracking-wide">{{ $t('socialLogin.orContinueWith') || 'Or continue with' }}</span>
			<div class="flex-1 h-px bg-gray-200 dark:bg-slate-600"></div>
		</div>
		...
		<div class="flex gap-3">
			...
			<button
				...
				class="flex-1 flex items-center justify-center h-11 px-4 text-[13px] font-medium text-gray-600 dark:text-slate-300 bg-transparent border border-gray-300 dark:border-slate-600 rounded-md cursor-pointer transition-all duration-200 hover:bg-gray-50 hover:border-gray-400 dark:hover:bg-slate-700 dark:hover:border-slate-500 active:scale-[0.98]"
			>
				<svg
					class="w-[18px] h-[18px] mr-2 flex-shrink-0 opacity-90"
				...
				<span class="truncate">{{ $t('socialLogin.wechatWork') }}</span>
			</button>
			...
			>
				...
				<span class="truncate">{{ $t('socialLogin.dingtalk') }}</span>
			</button>
		</div>
	</div>
</template>
