...
<template>
  <div v-loading="mainLoading"
    ...
    <i v-if="!outTime"
       :class="objData.amrPlaying != true ? 'icon-yuyin': 'icon-playing1'">
      <span class="amr-duration"
            v-if="objData.amrDuration">{{objData.amrDuration}}"</span>
    </i>
    <i v-if="outTime"
       class="el-icon-time">
    ...
    <div v-if="objData.repContent">
      <el-tag type="success"
              size="mini">语音识别</el-tag>{{objData.repContent}}
    </div>
  </div>
</template>
